<template>
    ...
    <div>
            ...
            <table class="table table-hover table-bordered">
                ...
                <tbody v-for="(user,index) in users" :key="index">
                    <tr>
                        ...
                        <!-- <td style=""><button type="button" class="btn btn-outline-primary" @click="openModel(user.id)" data-toggle="modal" data-target="#exampleModalDelete"><i class="fa-solid fa-calendar-day"></i> View</button></td> -->
                        <td style="" v-if="(confirms.length > 0)"><button v-if="confirms.some(confirm => confirm.id_user === user.id)" type="button" class="btn btn-outline-success" @click="confirmUpdate(user.id)" data-toggle="modal" data-target="#modalUserDetails">Confirm</button></td>
                    ...
                    <div class="modal-body">
                        <form @submit.prevent="register()" class="col-12">
                            <div class="form-group">
                                ...
                                <input autocomplete="off" type="email" minlength="16" required v-model="user.email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
        ...
        <!-- Model Users Detail -->
        <div style="background-color: #3d3d3d99;" class="modal fade" id="modalUserDetails" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div style="max-width: 60%;" class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Confirm Information User</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" id="btnCloseUser">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body justify-content-center">
                        <UserDetails :user_details="user_details"></UserDetails>
                    </div>
                    <div class="modal-footer d-flex justify-content-center">
                        <button type="button" class="btn btn-outline-success" @click="confirmUpdateUser"><i class="fa-solid fa-circle-check"></i> Confirm Information</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted(){
        ...
        //     this.number1++;
        // }, 1000);
        window.document.title='User Management - PBL5';
        ...
        BaseRequest.get('unconfirms/')
        .then( data => {
            this.confirms = data.results;
            // console.log(this.confirms);
            // console.log(this.confirms.some(confirm => confirm.id_user === 4));
        })
        .catch( () => {
        })
    },
    methods: {
        ...
        //         });
        // }
        confirmUpdate:function(id_user){

            this.user_details = this.users.find(user => user.id === id_user);
            const { emitEvent } = useEventBus();
            emitEvent('ShowConfirmUser',this.user_details);

        },
        confirmUpdateUser:function(id_user){
            // window.localStorage.setItem('confirm_user', JSON.stringify(user_details));
            var id_user = this.user_details.id; 
            var result = this.confirms.find(confirm => confirm.id_user === id_user);
            // console.log(result.id); 
            BaseRequest.delete('unconfirms/'+result.id+'/')
            .then( () => {
                var close_btn = window.document.getElementById('btnCloseUser');
                close_btn.click();
                this.confirms = this.confirms.filter(confirm => confirm.id_user !== id_user); // xóa khỏi mảng 
                const { emitEvent } = useEventBus();
                emitEvent('eventSuccess','Confirm Update Success !');
            })
            .catch( () => {
                const { emitEvent } = useEventBus();
                emitEvent('eventError','Confirm Update Fail !');
            })
        }
    }
}
